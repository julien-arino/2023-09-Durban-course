...
\begin{center}
  \begin{tikzpicture}[scale=1.25, transform shape]
    %% Regular nodes
    \node [circle, fill=green!50, text=black] at (0\skip,0) (S) {$S$};
    \node [circle, fill=red!90, text=black] at (1\skip,0) (I) {$I$};
    \node [circle, fill=blue!90, text=black] at (2\skip,0) (R) {$R$};
    %% Fake nodes for arrows
    \node [left=1cm of S] (birth) {};
    \node [below=0.5cm of S] (dS) {};
    \node [below=0.5cm of I] (dI) {};
    \node [below=0.5cm of R] (dR) {};
    %% Flows
    \path [line, very thick] (birth) to node [midway, above] (TextNode) {$b(N)$} (S);
...
\begin{center}
  \begin{tikzpicture}[scale=1.25, transform shape]
    ...
    \path [line, very thick] (S) to node [midway, above] (TextNode) {$\beta SI$} (I);
    \path [line, very thick] (I) to node [midway, above] (TextNode) {$\gamma I$} (R);
    \draw [>=latex,->, thick, rounded corners] (R) -- (2\skip,0.75) -- (0,0.75) node[sloped, midway, above] {$\nu R$} -- (S);
  \end{tikzpicture}    
\end{center}
\vfill
\begin{center}
...
\begin{frame}{Invariance of $\mathbb{R}_+^2$ under the flow (1)}
...
\vfill
In this case, we say the EEP is \defword{biologically relevant}
\vfill
Otherwise the EEP is \defword{not biologically relevant} and by abuse of language, we often say it \textbf{does not exist}
\end{frame}
