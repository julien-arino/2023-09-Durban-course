...
\vfill
Differential equationsL unknowns are \emph{functions} (instead of scalars, like in algebraic equations)
\vfill
...
\begin{subequations}\label{sys:KMK}
  \begin{align}
    S' &= -\beta SI \label{sys:KMK_dS} \\
    I' &= \beta SI-\gamma I \label{sys:KMK_dI} \\
    R' &= \gamma I \label{sys:KMK_dR}
    \end{align}  
\end{subequations}
\vfill
\begin{center}
