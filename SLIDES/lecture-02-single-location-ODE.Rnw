...
\bbullet Paper we have already seen: Arino, Brauer, PvdD, Watmough \& Wu. \href{http://dx.doi.org/10.1098/rsif.2006.0112}{Simple models for containment of a pandemic} (2006)
\vfill
...
\begin{frame}
\centering
\includegraphics[width=\textwidth]{FIGS/SLIR_epidemic_with_vaccination}
\end{frame}  
...
Arino, Bowman \& Moghadas. \href{https://bmcinfectdis.biomedcentral.com/counter/pdf/10.1186/1471-2334-9-8.pdf}{Antiviral resistance during pandemic influenza: implications for stockpiling and drug use}. BMC Infectious Disease (2009)
\vfill
...
\resizebox{\textwidth}{!}{
  \begin{tikzpicture}[%transform canvas={scale=1.3},
    ...
    \path [line, very thick] (S) to node [midway,above,sloped] (TextNode) {$qpfS$} (I_T);
...
\begin{frame}{The original model}
\centering
\begin{tikzpicture}[auto, %node distance = 2cm, auto,
	cloud/.style={minimum width={width("N-1")+2pt},
		draw, ellipse,fill=red!20}]
	\node [cloud] (S) {$S$};
	\node [cloud, right=of S] (L1) {$L_1$};
	\node [cloud, right=of L1] (L2) {$L_2$};
	\node [cloud, above right=of L2, fill=blue!20] (I1) {$I_1$};
	\node [cloud, below right=of L2] (A1) {$A_1$};
	\node [cloud, right=of I1, fill=blue!20] (I2) {$I_2$};
	\node [cloud, right=of A1] (A2) {$A_2$};
	\node [cloud, below right=of I2, fill=blue!20] (RI) {$R_I$};
	\node [cloud, right=of A2] (RA) {$R_A$};
	\node [cloud, above right=of I2, fill=blue!20] (D) {$D$};
	%% Infections
	...
	\path [line, very thick] (L2) to node [midway, right] (TextNode) {$(1-\pi)\varepsilon L_2$} (I1);
	\path [line, very thick] (L2) to node [midway, right] (TextNode) {$\pi\varepsilon L_2$} (A1);
	...
	\path [line, very thick] (I2) to node [midway, right] (TextNode) {$(1-\delta)\gamma I_2$} (RI);
	\path [line, very thick] (A2) to node [midway, above] (TextNode) {$\gamma A_2$} (RA);
	\path [line, very thick] (I2) to node [midway, right] (TextNode) {$\delta\gamma I_2$} (D);
\end{tikzpicture}
\end{frame}
...
\centering
\includegraphics[width=0.95\textwidth]{FIGS/SIRV_bif_forward}
\end{frame}

\begin{frame}{What can happen with vaccination -- Backward bifurcation}
\centering
\includegraphics[width=0.95\textwidth]{FIGS/SIRV_bif_backward}
\end{frame}
