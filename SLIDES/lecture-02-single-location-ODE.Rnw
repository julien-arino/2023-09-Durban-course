...
\begin{align}
S' &= b +\gamma I -dS - \beta SI \label{sys:SIS_dS} \\
I' &= \beta SI -(d+\gamma) I
\end{align}
\end{subequations}
...
\begin{align}
\bS' &= \mathbf{b}(\bS,\bI,\bR)-\bD\bS\beta(\bS,\bI,\bR)\bh\bI \label{sys:SIR_general_dS} \\
\bI' &= \bPi\bD\bS\beta(\bS,\bI,\bR)\bh\bI-\mathbf{V}\bI \label{sys:SIR_general_dI} \\
\bR' &= \mathbf{f}(\bS,\bI,\bR)+\mathbf{W}\bI \label{sys:SIR_general_dR}
\end{align}
\end{subequations}
\vfill
...
\bS' = \mathbf{b}(\bS,\bI,\bR)-\bD\bS\beta(\bS,\bI,\bR)\bh\bI
\end{equation}
\begin{itemize}
...
\bI' = \bPi\bD\bS\beta(\bS,\bI,\bR)\bh\bI-\mathbf{V}\bI
\end{equation}
\begin{itemize}
...
\bR' = \mathbf{f}(\bS,\bI,\bR)+\mathbf{W}\bI
\end{equation}
\begin{itemize}
...
\begin{align*}
\bS' &= \mathbf{b}(\bS,\b0,\bR) \\
\bR' &= \mathbf{f}(\bS,\b0,\bR) \\
\end{align*}
...
\begin{subequations}
\begin{align}
\bS' &= -\bD\bS\beta(\bS,\bI,\bR)\bh\bI  \label{sys:SIR_epi_dS} \\
\bI' &= \bPi\bD\bS\beta(\bS,\bI,\bR)\bh\bI-\mathbf{V}\bI 
...
\resizebox{\textwidth}{!}{
  ...
  \begin{tikzpicture}[%transform canvas={scale=1.3},
    ...
    \path [line, dotted, very thick] (IV) to node [midway,below,sloped] (TextNode) {} (SH);
    \path [line, dotted, very thick] (IH) to node [midway,above,sloped] (TextNode) {} (SV);
    \draw [>=latex,->, thick, rounded corners] (IH) -- (1\horskip,0.75) -- (0,0.75) node[midway,above,sloped] {$\nu R$} -- (SH);
